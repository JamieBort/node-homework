...
exports.register = async (req, res, next) => {
	...
	try {
		...
		if (err.name === "PrismaClientKnownRequestError" && err.code === "P2002") {
			// TODO: send the appropriate error back -- the email was already registered
			return res
				.status(400)
...
exports.logon = async (req, res) => {
	...
	// Find user by email
	email = email.toLowerCase(); // Joi validation always converts the email to lower case. But you don't want logon to fail if the user types mixed case. Also, Prisma findUnique (below) can't do a case insensitive search.
